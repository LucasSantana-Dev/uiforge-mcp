...
export function getAvailableHeadlessComponents(): string[] {
  return getHeadlessTemplates().map(t => t.name).concat([
    'Listbox', 'Menu', 'Popover', 'RadioGroup',
    'Switch', 'Disclosure', 'FocusTrap', 'TransitionGroup',
  ]);
}

export function getAvailableHeadlessPatterns(): string[] {
  return [
    'CommandPalette', 'MultiSelect', 'NestedMenu',
    'Slideover', 'Notification', 'StepWizard',
  ];
}
