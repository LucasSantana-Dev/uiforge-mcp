...
export const cardSnippets: IComponentSnippet[] = [
  ...
  <div className="mt-4 flex gap-2">
    ...
    tailwindClasses: {
      ...
      button: 'rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 active:scale-[0.98] transition-all',
      buttonSecondary: 'rounded-lg border border-input px-4 py-2 text-sm font-medium text-foreground hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 active:scale-[0.98] transition-all',
    },
    a11y: { roles: ['article'], ariaAttributes: [], keyboardNav: 'Tab through interactive elements', contrastRatio: '4.5:1', focusVisible: true, reducedMotion: true },
    seo: { semanticElement: 'article' },
    responsive: { strategy: 'mobile-first', breakpoints: ['sm', 'md'] },
    quality: {
      antiGeneric: ['rounded-xl for modern feel', 'hover:shadow-md for depth on interaction', 'article element for semantics', 'active:scale on buttons'],
  ...
  <div className="mt-3">
    ...
    a11y: { roles: [], ariaAttributes: ['aria-hidden on icons'], keyboardNav: 'N/A — informational', contrastRatio: '4.5:1', focusVisible: false, reducedMotion: true },
    seo: { semanticElement: 'div' },
    responsive: { strategy: 'mobile-first', breakpoints: [] },
    quality: {
      antiGeneric: ['trend indicator with icon + text — not color-only', 'tracking-tight on large number', 'contextual icon in header', 'dark mode trend color'],
  ...
  <button type="button" className="mt-8 w-full rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 active:scale-[0.98] transition-all">
    ...
    a11y: { roles: ['list', 'listitem'], ariaAttributes: ['role="list"', 'aria-hidden on check icons'], keyboardNav: 'Tab to CTA button', contrastRatio: '4.5:1', focusVisible: true, reducedMotion: true },
    seo: { semanticElement: 'div' },
    responsive: { strategy: 'mobile-first', breakpoints: ['sm', 'md'] },
    quality: {
      antiGeneric: ['rounded-2xl for premium feel', 'check icons are primary color', 'tracking-tight on price for impact', 'hover:shadow-lg for depth'],
  ...
  <figcaption className="mt-4 flex items-center gap-3">
    ...
    tailwindClasses: {
      ...
      avatar: 'h-10 w-10 rounded-full bg-muted flex items-center justify-center text-sm font-medium text-muted-foreground',
      name: 'text-sm font-semibold text-foreground',
      role: 'text-xs text-muted-foreground',
    },
    a11y: { roles: ['figure'], ariaAttributes: [], keyboardNav: 'N/A — content', contrastRatio: '4.5:1', focusVisible: false, reducedMotion: true },
    seo: { semanticElement: 'figure' },
    responsive: { strategy: 'mobile-first', breakpoints: [] },
    quality: {
      antiGeneric: ['figure + figcaption for proper semantics', 'avatar fallback initials', 'leading-relaxed for quote readability'],
  ...
  <p className="mt-2 text-sm text-muted-foreground leading-relaxed">Built for speed with optimized rendering and minimal bundle size for the best user experience.</p>
    ...
    tailwindClasses: {
      ...
      iconWrapper: 'mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground',
      icon: 'h-5 w-5',
      title: 'text-base font-semibold text-foreground',
      description: 'mt-2 text-sm text-muted-foreground leading-relaxed',
    },
    a11y: { roles: [], ariaAttributes: ['aria-hidden on icon'], keyboardNav: 'N/A — content', contrastRatio: '4.5:1', focusVisible: false, reducedMotion: true },
    seo: { semanticElement: 'div' },
    responsive: { strategy: 'mobile-first', breakpoints: [] },
    quality: {
      antiGeneric: ['group hover transforms icon wrapper', 'hover:-translate-y-0.5 lift effect', 'icon bg transitions from primary/10 to primary on hover — reveals intent'],
